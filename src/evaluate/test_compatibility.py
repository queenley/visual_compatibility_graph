...
def test_compatibility(_args):
    ...
    dl.setup_test_compatibility(resampled=_args.resampled)
    ...
    def eval():
        ...
        val_avg_loss, val_acc, conf, _pred = sess.run([model.loss, model.accuracy, model.confmat, model.predict()],
    ...
    with tf.Session() as sess:
        ...
        # evaluate the model for accuracy prediction
        eval()
        ...
        for outfit in tqdm(dl.comp_outfits):
            ...
            if _args.subset:  # use only a subset (of size 3) of the outfit
            ...
            new_support = get_degree_supports(new_adj, config['degree'], adj_self_con=ADJ_SELF_CONNECTIONS,
                                              verbose=False)
